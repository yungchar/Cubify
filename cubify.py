def cubify(s):
    ...
    # Step 1. The top horizontal line: It's (len(s) - 1) spaces, followed by s backwards.
    result += " " * (len(s) - 1)
    result += s[::-1]
    result += "\n"
    ...
    #   (len(s) - x) spaces + xth last char + (len(s) - 2) spaces + xth char + (x - 2) spaces + xth char
    ...
    # Step 3. The middle horizontal line: The string forwards + (len(s) - 2) spaces + last char
    result += s
    result += " " * (len(s) - 2)
    result += s[-1]
    result += "\n"
    ...
    #   xth char + (len(s) - 2) spaces + xth last char + (len(s) - x - 2) spaces + xth last char
    # repeated (len(s) - 2) times.
    for i in range(2, len(s)):
        result += s[i - 1]
        result += " " * (len(s) - 2)
